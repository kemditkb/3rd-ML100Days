...
for var_columns in var_train_columns:
    var_temp_data = df_cp[var_columns].dropna()
    q25, q75 = np.percentile(var_temp_data, 25), np.percentile(var_temp_data, 75)
    iqr = q75 - q25
    #print('Percentiles: 25th=%.3f, 75th=%.3f, IQR=%.3f' % (q25, q75, iqr))
     # calculate the outlier cutoff
    cut_off = iqr * 3
    lower, upper = q25 - cut_off, q75 + cut_off
    df_cp[var_columns] = df_cp[var_columns].clip(lower, upper)
...
y_pred_proba =  gdbt.predict_proba(x_selected_test)[:,1]
...
########### 'max_depth': 4, 'n_estimators': 100, 'tol': 20
gdbt = GradientBoostingClassifier(tol=20, subsample=0.75, n_estimators=100, max_features=x_selected.shape[1], 
...
update_y_pred = gdbt.predict_proba(x_selected_test)[:,1]

acc = accuracy_score(y_test, y_pred)
print("Accuracy: ", acc)
var_confusion_matrix = confusion_matrix(y_test, y_pred, labels=None, sample_weight=None)
check_view = pd.DataFrame({'pred_poi': update_y_pred,'poi':y_test})
check_view = check_view.sort_values(by=['pred_poi'])
...
var_key = ['total_stock_value','salary','total_payments','exercised_stock_options','other','bonus']
...
sub.to_csv('test1027_gdbt.csv', index=False)
