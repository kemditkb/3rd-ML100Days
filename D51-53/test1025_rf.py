...
df_cp['deferred_income'] = df_cp['deferred_income'].fillna(df_cp['deferred_income'].median())
df_cp['restricted_stock'] = df_cp['restricted_stock'].fillna(df_cp['restricted_stock'].median())
df_cp['total_payments'] = df_cp['total_payments'].fillna(df_cp['total_payments'].median())



df_cp['bonus'] = df_cp['bonus'].fillna(df_cp['bonus'].mean())
df_cp['deferral_payments'] = df_cp['deferral_payments'].fillna(df_cp['deferral_payments'].mean())
df_cp['exercised_stock_options'] = df_cp['exercised_stock_options'].fillna(df_cp['exercised_stock_options'].mean())
df_cp['long_term_incentive'] = df_cp['long_term_incentive'].fillna(df_cp['long_term_incentive'].mean())
df_cp['total_stock_value'] = df_cp['total_stock_value'].fillna(df_cp['total_stock_value'].mean())
...
gdbt = GradientBoostingClassifier(tol=160, subsample=0.75, n_estimators=130, max_features=x_selected.shape[1], 
...
param_test1 = {'n_estimators':range(100,400,30),'tol':range(140,200,20),'max_depth':range(1,5,1)}
...
var_key = ['total_stock_value','total_payments','long_term_incentive','other','bonus','exercised_stock_options']
update_test_x = df_cp[var_key]

gdbt_pred = gdbt.predict_proba(update_test_x)[:,1]

sub = pd.DataFrame({'name': ids, 'poi': gdbt_pred})
sub.to_csv('test1025-4_gdbt.csv', index=False)
